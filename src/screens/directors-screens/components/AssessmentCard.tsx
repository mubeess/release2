...
import useAssessmentTrackingGet from "../hooks/useAssessmentTrackingGet";
interface IProps {
  termId: string;
  activeLevels: ClassLevelDto[];
}
export default function AssessmentCard({ termId, activeLevels }: IProps) {
  const {
    loading,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.head}>
        <Text h2>Assessment Configured</Text>
      ...
      <View style={styles.head}>
        ...
        >
          {levelsConfigured?.length} / {levelsUsingAssessment?.length}
        </Text>
