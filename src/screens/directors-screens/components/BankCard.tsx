...
import useBankSettingsTrack from "../hooks/useBankSettingsTrack";
import BankRecordItem from "./BankRecordItem";
interface IProps {
  termId: string;
  activeLevels: ClassLevelDto[];
}
export default function BankCard({ termId, activeLevels }: IProps) {
  const {
    loading,
    bankAccounts,
    activeLevels: classLevels,
    levelsConfigured,
    isComplete,
    percentageConfigured,
    levelConfigurations,
  } = useBankSettingsTrack();
    ...
    <>
      <View style={styles.container}>
        <View style={styles.head}>
          <Text h2>Bank Configured</Text>
          <Text style={{ color: lightTheme.colors.PrimaryGrey }}>
            Total bank accounts configured for different class levels
          </Text>
        </View>
        ...
        <View style={styles.head}>
          ...
          >
            {levelsConfigured?.length} / {classLevels?.length}
          </Text>
        ...
        >
          <View style={styles.parentModal}>
            <View style={styles.mainDetail}>
              ...
              >
                Below are the list of class levels and their configuration
                status
              </Text>
              <ScrollView
      ...
      <View style={styles.banks}>
        <View style={styles.heading}>
          <Text h2>Class Level</Text>
          <Text h2>Bank Name</Text>
        </View>

        {bankAccounts.map((banks) => (
          <BankRecordItem
            key={banks.id}
            data={banks}
            bank={banks.bank?.name}
            level={banks.class_level?.short_name || ""}
          />
        ))}
      </View>
    </>
